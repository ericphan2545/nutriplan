...
const BMIChecker = {
    ...
    // BMI Categories
    categories: {
        underweight: { max: 18.5, status: 'Thiếu cân', class: 'status-underweight' },
    ...
    // Advice messages
    adviceMessages: {
        underweight: 'Bạn đang thiếu cân. Hãy tăng cường bổ sung dinh dưỡng với các thực phẩm giàu protein và carbohydrate lành mạnh. Ăn nhiều bữa nhỏ trong ngày và tập thể dục để tăng cơ.',
        normal: 'Chúc mừng! Bạn có chỉ số BMI lý tưởng. Hãy duy trì chế độ ăn uống cân bằng và tập thể dục đều đặn để giữ gìn sức khỏe.',
        overweight: 'Bạn đang thừa cân. Hãy giảm lượng calo nạp vào, tăng cường rau xanh và protein, hạn chế đường và tinh bột. Tập thể dục ít nhất 30 phút mỗi ngày.',
        obese: 'Bạn đang ở mức béo phì. Cần có kế hoạch giảm cân nghiêm túc với chế độ ăn kiêng và tập luyện. Nên tham khảo ý kiến bác sĩ hoặc chuyên gia dinh dưỡng.'
    },

    // Food suggestions by BMI category
    foodSuggestions: {
        underweight: ['Phở bò', 'Cơm tấm', 'Bánh mì thịt', 'Bún chả', 'Cháo gà', 'Trứng chiên', 'Sữa chua', 'Chuối', 'Bơ', 'Hạt óc chó'],
        normal: ['Gỏi cuốn', 'Canh chua', 'Cá hấp', 'Salad', 'Phở gà', 'Rau luộc', 'Trái cây', 'Đậu hũ', 'Cơm gạo lứt', 'Súp rau củ'],
        overweight: ['Salad rau', 'Cá hấp', 'Ức gà nướng', 'Rau luộc', 'Canh rau', 'Đậu hũ', 'Trứng luộc', 'Dưa leo', 'Cà chua', 'Bông cải xanh']
    },
    ...
    updateStatusDisplay(category) {
        ...
        if (statusEl) {
            const categoryInfo = this.categories[category === 'obese' ? 'obese' : category];
    ...
    calculateNutrition(bmi, category) {
        ...
        if (this.userData.gender === 'male') {
            bmr = 10 * this.userData.weight + 6.25 * this.userData.height - 5 * 25 + 5;
        } else {
            bmr = 10 * this.userData.weight + 6.25 * this.userData.height - 5 * 25 - 161;
        }
    ...
    getMacroRatios(category) {
        const ratios = {
            underweight: { protein: 0.20, carbs: 0.50, fat: 0.30 },
            normal: { protein: 0.25, carbs: 0.45, fat: 0.30 },
            overweight: { protein: 0.30, carbs: 0.35, fat: 0.35 },
            obese: { protein: 0.30, carbs: 0.35, fat: 0.35 }
        };
    ...
    updateFoodSuggestions(category) {
        ...
        const foods = this.foodSuggestions[foodCategory] || this.foodSuggestions.normal;
